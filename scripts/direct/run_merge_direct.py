...
class Paths:
    ...
    def __init__(self, out_root, plate, well):
        ...
        if zarr_probe.exists():
            stem = f"{plate}/{r}/{c}"
            self._parq = (
                lambda subdir, name: self.root / subdir / stem / f"{name}.parquet"
            )
            self._tsv = lambda subdir, name: self.root / subdir / stem / f"{name}.tsv"
        else:
            pfx = f"P-{plate}_W-{well}__"
            self._parq = (
                lambda subdir, name: self.root / subdir / f"{pfx}{name}.parquet"
            )
...
def step_fast_alignment(cfg, paths, plate, well, workers, force):
    ...
    ph_xy = ph_meta.rename(columns={"x_pos": "x", "y_pos": "y"}).set_index("tile")[
        ["x", "y"]
    ]
    sbs_xy = sbs_meta.rename(columns={"x_pos": "x", "y_pos": "y"}).set_index("tile")[
        ["x", "y"]
    ]

    log("fast_alignment: hashing cell locations")
    ph_hash = validate_dtypes(hash_cell_locations(ph_info))
    sbs_hash = validate_dtypes(
    ...
    if (initial_sbs_tiles is None) == (initial_sites_param is None):
        raise ValueError(
            "Exactly one of 'initial_sbs_tiles' or 'initial_sites' must be set in merge config"
        )
    d0, d1 = cfg["det_range"]
    ...
    if initial_sbs_tiles is not None:
        ...
        log(
            f"fast_alignment: {len(candidate_pairs)} candidate pairs from {len(initial_sbs_tiles)} SBS tiles"
        )
    else:
    ...
    initial_alignment_df = initial_alignment(
        ph_hash, sbs_hash, initial_sites=candidate_pairs
    )
    valid = initial_alignment_df.query(
    ...
    log(
        f"fast_alignment: {len(initial_sites)} initial sites passed thresholds; running multistep_alignment (n_jobs={workers})"
    )
    ...
    log(
        f"fast_alignment: wrote {len(well_alignment)} alignments -> {paths.fast_alignment}"
    )
...
def step_fast_merge(cfg, paths, force):
    ...
    fa["rotation"] = [
        np.array([r1, r2]) for r1, r2 in zip(fa["rotation_1"], fa["rotation_2"])
    ]
    ...
    for _, row in fa_f.iterrows():
        ...
        merge_data.append(
            merge_triangle_hash(ph_f, sbs_f, row, threshold=cfg["threshold"])
        )
...
def step_format_merge(cfg, paths, force):
    ...
    mf = merge_data.pipe(
        fov_distance, i="i_0", j="j_0", dimensions=ph_dims, suffix="_0"
    )
...
def step_deduplicate_merge(cfg, paths, force):
    ...
    dedup, stats = deduplicate_cells(
        ...
        return_stats=True,
        approach=approach,
        sbs_dedup_prior=cfg.get("sbs_dedup_prior"),
        pheno_dedup_prior=cfg.get("pheno_dedup_prior"),
    )
    da = analyze_distance_distribution(dedup)
    if da:
        s = da["distance_stats"]
        log(
            f"deduplicate_merge: mean dist {s['mean']:.2f}px, median {s['median']:.2f}px, n={len(dedup)}"
        )
    ...
    log(
        f"deduplicate_merge: wrote {len(dedup)} deduplicated cells -> {paths.merge_deduplicated}"
    )
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Direct merge runner (fast chain, bypasses Snakemake)"
    )
    ap.add_argument("--config", required=True)
    ap.add_argument(
        "--workers", type=int, default=8, help="n_jobs for multistep_alignment"
    )
    ...
    for _, row in combos.iterrows():
        run_well(
            cfg, out_root, row["plate"], str(row["well"]), args.workers, args.force
        )
