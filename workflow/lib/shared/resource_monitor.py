...
def _pmon_util_by_pid(out):
    """Parse `nvidia-smi pmon -c 1` output into {pid: sm_util_pct}.
...
def _compute_apps_mem_by_pid(out):
    """Parse `nvidia-smi` compute-apps output into {pid: used_mem_mb}.

    Expects `--query-compute-apps=pid,used_memory
    --format=csv,noheader,nounits`. '[N/A]' rows are skipped.
    """
    mem = {}
    for line in out.splitlines():
        line = line.strip()
...
class ResourceMonitor:
    ...
    def _write(self):
        ...
        row = {
            ...
            "mean_proc_gpu_pct": round(self._proc_gpu_sum / self._proc_gpu_samples, 1)
            if self._proc_gpu_samples
            else 0.0,
            "max_proc_gpu_mem_mb": round(self.max_proc_gpu_mem, 1),
            "mean_proc_gpu_mem_mb": round(
                self._proc_gpu_mem_sum / self._proc_gpu_samples, 1
            )
            if self._proc_gpu_samples
            else 0.0,
        }
